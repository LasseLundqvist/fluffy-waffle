...
- Training window starts at BACKTEST_START
- Minimum MIN_TRAIN_MONTHS observations before first forecast
- Each period: fit on history, forecast next period, expand window
...
from cpi_nowcast.config import BACKTEST_START, MIN_TRAIN_MONTHS, MODEL_CFG
from cpi_nowcast.evaluation.metrics import evaluate_all, rmse
...
def random_walk_forecast(y: pd.Series) -> pd.Series:
    """
    Random walk benchmark: CPI_yoy(t) = CPI_yoy(t-1).

    Returns a forecast series aligned with y (one-step-ahead).
    """
...
def run_backtest(
    ...
    for t in forecast_dates:
        ...
        try:
            ...
            if not X_pred.empty:
                pred = bridge.predict(X_pred)
                record["bridge"] = float(pred.iloc[0]) if not pred.isna().all() else np.nan
            else:
    ...
    # ── Ensemble (uses bridge + midas historical predictions) ────

...
Implements RMSE, MAE, MFE (bias), and the Diebold-Mariano test.
"""
...
def skill_score(
    ...
    """
    Compute skill score relative to a benchmark.
...
def diebold_mariano_test(
    ...
    actuals : pd.Series
    forecasts_1 : pd.Series
        Model 1 (being tested).
    forecasts_2 : pd.Series
        Benchmark / Model 2.
    loss : str
        'squared' or 'absolute'.
    h : int
        Forecast horizon (used for HAC variance estimation).
    ...
    gamma = [np.cov(d.values[:-j] if j > 0 else d.values, d.values[j:] if j > 0 else d.values)[0, 1]
             for j in range(h)]
...
def evaluate_all(
    ...
    forecasts : pd.Series
        Model nowcasts.
    benchmark_rw : pd.Series, optional
        Random walk benchmark (CPI_yoy(t) = CPI_yoy(t-1)).
    benchmark_ar : pd.Series, optional
        AR(1) benchmark.
    label : str
